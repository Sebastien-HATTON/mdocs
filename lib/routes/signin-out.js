...
function handleOtherErrors (req, res, next) {
  if (!req.query.error) return next();
  var description = req.query.error_description;
  if (!description) {
    description = req.query.error === 'access_denied' ? 'access denied' : 'unknow error';
  }
...
module.exports = function(app){
  ...
  app.get('/e/:connection', function (req, res) {
    var url = '/login?connection=' + req.params.connection;
    if(req.user){
      req.session.returnToAfterLogout = url;
