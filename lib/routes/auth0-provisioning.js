...
module.exports = function (app) {
  app.post('/provisioning', function(req, res) {
    ...
    provisioningRequest.name = provisioningRequest.domain.replace(/\./ig, '-').replace(/(-com)$/, '');
    ...
    switch (provisioningRequest.strategy) {
      ...
      case 'Google Apps':
        ...
        provisioningRequest.options.client_id = process.env.gapps365_clientId;
        provisioningRequest.options.client_secret = process.env.gapps365_clientSecret;
        provisioningRequest.options.ext_profile = true;
        provisioningRequest.options.api_enable_users = true; 
        break;
    }
