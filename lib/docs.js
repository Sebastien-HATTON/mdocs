...
docs.getMaxPermission = function (user, doc) {
  if( user ){
    ...
    if (user.identities && 
        ...
        doc.visibility && 
        doc.visibility.companies[user.companyId]) {
      return doc.visibility.companies[user.companyId];
    }
  }
...
docs.getAll = function (user, options, done) {
  ...
  if(user.identities && !user.identities[0].isSocial){
    ...
    companyWideFilter['visibility.companies.' + user.companyId] = {$exists: true};
