...
function MongoDBStore (options, docId, callback) {
  ...
  this._get_collection = function(callback) {
    if (self.collection) {
      if (callback) callback(self.collection);
    } else {
...
MongoDBStore.prototype.getDocument = function (options, callback) {
  var self = this;
  ...
  this._get_collection(function (collection) {
    if(!('since' in options)){
        ...
        collection.findOne({_id: self.docId}, fields, function (err, doc) {
          if (err) return callback(err);
          if (!doc) return callback(null, "", 0);
          doc.operations = doc.operations.map(mapOperation);
          return callback(null, doc);
        });
    } else {
      collection.aggregate([
        {$match: {_id: self.docId}}, 
        {$unwind: '$operations'}, 
        {$skip:   options.since}
      ], function (err, result){
        if(err) return callback(err);
        ...
        if(result.length === 0){
          //the document has no newer operations
          return collection.findOne({_id: self.docId}, {
            operationsCount: 1, 
            content:         1, 
            lastUpdate:      1
          }, function (err, doc) { 
        ...
        var operations = (result || []).map(function(r) {
          return r.operations;
        }).map(mapOperation);
        
        var doc = {
          operationsCount: result[0].operationsCount, 
          content:         result[0].content,
          lastUpdate:      result[0].lastUpdate,
          operations:      operations
        };
        console.log('doqui', doc);
        callback(null, doc);
      });
    }
  });
...
/**
 * Insert new operation and modify the content of a document 
 * @param  {Object}   operation  [description]
 * @param  {Date}     lastUpdate [description]
 * @param  {String}   content    [description]
 * @param  {Function} callback   [description]
 */
MongoDBStore.prototype.insertOperation = function (
    operation, 
    lastUpdate, 
    content, 
    callback) {
  ...
  try{
    newContent = operation.apply(content || '');
  } catch(er) {
    console.log('Error aplying the operation', JSON.stringify(operation), '\n Error', er, '\n' + content);
    return callback(er);
  }
  ...
  self._get_collection(function (collection) {
    collection.update({ 
      ...
      $inc: { operationsCount: 1 }
    }, { upsert: true }, function (err, count) {
      if (count === 0 || (err && err.code === 11000)) {
        console.log('probabily ... concurrency error. retry please. Count:', count, 'Error:', err);
